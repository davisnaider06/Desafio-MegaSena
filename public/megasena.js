...
async function load(concurso) {
  let url = location.origin;  // Pega automaticamente o domínio atual
...
function preencherHTML(result) {
  ...
  if (result.ganhadores_6_acertos == 0) {
    ...
    document.getElementById("ganhadores-6-acertos").textContent = 
      `${result.ganhadores_6_acertos} aposta(s) ganhadora(s), ${formatReal(result.rateio_6_acertos)}`;
  }

  document.getElementById("acumulado-sorteio-especial-mega-da-virada").textContent = 
    formatReal(result.acumulado_sorteio_especial_mega_da_virada);

  document.getElementById("ganhadores-5-acertos").textContent = 
    `${result.ganhadores_5_acertos} apostas ganhadoras, ${formatReal(result.rateio_5_acertos)}`;

  document.getElementById("ganhadores-4-acertos").textContent = 
    `${result.ganhadores_4_acertos} apostas ganhadoras, ${formatReal(result.rateio_4_acertos)}`;
